...
int main(int argc, char **argv)
{
    try {
        ...
        if (argc != 4 && argc != 5)
        {
            ...
            cerr << "or:    "<<argv[0]<<" [model.aspen] kernel param blockname" << endl;
            return 1;
        }
        ...
        string param = argv[3];
        string blockname = (argc==5) ? argv[4] : "";

        const ASTKernel *k = app->kernelMap[kernel];

        ASTExpression *dssize = k->GetExclusiveDataSizeExpression(app, blockname);
