...
int main(int argc, char **argv)
{
    try {
        ...
        ASTExpression *expr0 = k->GetTimeExpression(app, mach0, socket0);
        ASTExpression *expr1 = k->GetTimeExpression(app, mach1, socket1);

        ASTExpression *dssize = k->GetInclusiveDataSizeExpression(app);
        ASTExpression *xfer0 = mach0->GetMachine()->GetTimeExpression("intracomm",
                                                                     vector<string>(),
                                                                     false,
                                                                     socket0,
                                                                     dssize->Cloned());
        ASTExpression *xfer1 = mach1->GetMachine()->GetTimeExpression("intracomm",
                                                                     vector<string>(),
                                                                     false,
                                                                     socket1,
                                                                     dssize->Cloned());
        ...
        NameMap<const ASTExpression*> mach0_expansions(mach0->paramMap);
        NameMap<const ASTExpression*> mach1_expansions(mach1->paramMap);
        ...
        cout << "#define " << rt1 << " 0 + " << expr1->GetText(ASTExpression::C) << "" << endl;
