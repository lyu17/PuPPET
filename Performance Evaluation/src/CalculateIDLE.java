...
public class CalculateIDLE {
	...
	public static double getEnergy(String fileName, int idlerate) {
		
		double prate = (double)idlerate / 30;
		if(idlerate == 0) {
			prate = 1.0 / 67;
		}
...
//		int[][] submit = IOScanner.getData(path2);
		...
		for(int i=0; i<=endTime; i++) {
			talPow += curPow + 67 * prate * curUtils.elementAt(i);
	...
	public static double getDVFSEnergy(int[] threshold, String fileName, int idlerate) {
